...
@njit(fastmath=True)
def clifford_jump(n: int, x: np.ndarray, z: np.ndarray) -> np.ndarray:
    """A more 'radical' Clifford transformation for global mapping search."""
    N = x.shape[0]
    m = (n + 1) % N
    
    # Perform a sequence of row-ops or swaps to 'jump' to a different region
    # Swap row n and m
    x[n], x[m] = x[m].copy(), x[n].copy()
    z[n], z[m] = z[m].copy(), z[n].copy()
    
    # Then spread from n
    return spread_node(n, x, z)
